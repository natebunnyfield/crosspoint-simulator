...
# 2 fingers, swipes on 1 and 2 fingers in four directions, long presses on 1 and
# 2 fingers, pinch, rotation, shake -- in a LAYERED model: a global row for each,
# which the strip above the paper and the band below it may override for the six
# single-finger ones. There is no "on the paper": a landing point between the
# boundaries is simply one nothing overrides.
#
# The property this exists for is that EVERY SURVIVING DEFAULT REPRODUCES THE
# PREVIOUS BUILD. An install that never opens the setting must behave as build
# 157 did, and there is no way to see that it does not except by using the app
# for a while and noticing a gesture feels wrong. So every default is asserted
# against the live mapping it replaced, by name and by button, and an unwritten
# key (0), a junk value and a value from a future build all resolve to it.
#
# THE TWO DELIBERATE DIFFERENCES ARE ASSERTED BY NAME: the three-finger tap
# (which toggled zen) and the four-finger tap (power) went with the finger
# counts the trim removed, so the test pins both keys ABSENT from Root.plist,
# pins that nothing ships bound to POWER while POWER is still OFFERED, and pins
# that exactly one row -- the hold above the paper -- ships bound to the zen
# toggle. A silent re-add is the failure mode; a ruling reversed by accident is
# what it would look like.
#
# The owner's rulings are pinned as rulings, so that removing one is a conscious
# act: an unbound gesture fires nothing (and must NOT fall back to its default,
# the obvious way to write this wrong); two gestures may share one action, with
# no conflict detection; ANY binding may be cleared, the zen ones included and
# with no guard, because they are ordinary bindings ("zen is toggleable in
# settings. drop this concern."); and zen scope is a property of the GESTURE AND
# ITS ZONE, not of the action bound to it -- a regression guard against the
# plausible implementation that promotes any gesture holding the toggle to
# always-on, and against the subtler inverse, where an INHERITED action would
# take the global row's gate instead of the zone's.
#
...
# its registration domain from those same DefaultValues -- the actions each row
# offers, each row's title, the GROUP it sits in, the ORDER, and that no row
# configures a gesture ON the paper.
run gesture_bindings \
  c++ -std=c++17 -Iios -o "$OUT/gesture_bindings" tests/gesture_bindings_test.cpp

# ...and that the plist is still the header's PROJECTION. The 38 gesture rows in
# Root.plist are generated by tools/gen_gesture_plist.py, so the likely failure
# is not a typo but a stale run -- a header edited and the generator not
# re-run. The test above catches a drifted key, default, title, group or order;
# this catches the rest, including the FooterText prose that only lives in the
# generator. Cheap, and it is the difference between "the plist is checked" and
# "the plist is what the header says".
...
# THE ZEN READING GOAL (owner 2026-09-24): the last-minute decay window, the
# session that zen starting restarts, the stall cap, the one predicate that
# says whether a moment is reading (zen only), and the picture math. Every
# failure is a page that decays at the wrong time, or never, with nothing logged.
