...
exactly the kind of accident that goes away in a refactor.
"""
import pathlib, re, sys

root = pathlib.Path(__file__).resolve().parent.parent
pat = re.compile(r'^\s*#\s*(?:if|elif)\b.*\bFREEINK_[A-Z0-9_]+', re.M)
bad = []
for p in sorted(list((root / 'src').glob('*.[ch]*')) + list((root / 'ios').glob('*.[chm]*'))):
    if p.name == 'BoardConfig.h':
        continue
    text = p.read_text(errors='replace')
    if pat.search(text) and not re.search(r'#\s*include\s*[<"]BoardConfig\.h[>"]', text):
