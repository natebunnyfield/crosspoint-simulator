...
| ~~battery 100%, USB always connected~~ **FIXED 2026-08-08** — `CROSSPOINT_SIM_BATTERY=<0-100>`, `CROSSPOINT_SIM_USB=0`; default unchanged. Verified: at 7% unplugged the charging bolt is gone and the battery draws empty. **Half of it came back on iOS and was fixed again 2026-09-14**: both stubs latched the env var into a function-local `static`, and a phone has no environment to set, so the header showed 100 % and a permanent bolt beside iOS's own status bar. `src/SimHostBattery.h` publishes the phone's real level and charging state; nothing latches now, and `wasUsbStateChanged()` stopped being a flat `false` — it is what `main.cpp:1177` turns into the repaint that shows a plug between page turns | real gauge + GPIO | was: charging bolt always drawn, plug/unplug repaint never fires |
