...
| WHY this face is drawn with deliberate irregularities, and where the line is | [docs/albo-imperfections.md](docs/albo-imperfections.md) — owner's own phrase, *"imperfections that improve legibility"*. The claim, the four rules that rule out most ways of doing it (a TABLE never a jitter; never cut where the pen is already thick; 2–7 units at a 674 cap; an imperfection that costs legibility is a defect), what is actually cut letter by letter, the negative results, and the three cases that mark the edge |
| The HAIRLINE GAP the Y leaves at its join, and why the P now has one too | [docs/albo-hairline-gap.md](docs/albo-hairline-gap.md) — it was never designed; the owner saw what two strokes happened to leave, named it and made it a feature. Measured on both letters, with the five dials that cut it and the junctions it was deliberately NOT given to |
| What does a real italic do to its CAPITALS | [docs/albo-italic-capitals.md](docs/albo-italic-capitals.md) — 17 roman/italic pairs measured; they are NOT uprighter, NOT shorter and their serifs are NOT reduced; ~5% narrower and eight letters genuinely redrawn. `tools/wedge_serif/cmp_italic_caps.py` re-runs it. Research only, nothing drawn |
| What does the REFERENCE italic measure, letter by letter, in Albo's own design units — and how do I check a redrawn letter against it | [docs/albo-aldine-targets.md](docs/albo-aldine-targets.md) — Flanker Griffo Italic, Pagella and the scan crops measured UNSHEARED at xh 429 (`tools/wedge_serif/aldine_targets.py` regenerates it); Flanker's lowercase stem is 70 units = 0.83 S on 19 of 26 letters. `cmp_aldine_shape.py` overlays a built letter on its reference (scan crop first, then Poetica — the owner's preferred fallback, 2026-09-16) and reports IoU; `aldine_fit_shape.py` coordinate-descends a letter's env dials on that number; `aldine_proof.py` regenerates the proof page — two English paragraphs first, then the letter sheet |
| What is the binocular **g** made of, measured against the scan and the reference italics | [docs/albo-g-anatomy.md](docs/albo-g-anatomy.md) — the two counters, the neck's waist, and the ear's reach and slope, measured the SAME WAY on a raster whether the source is an outline or a photograph of a printed page; `tools/wedge_serif/cmp_aldine_g.py` re-runs it. Carries the two negative results: the scan's g touches the following i so it cannot measure the ear, and rooting the ear on the crown opens a notch the glitch gate cannot see |
| Is any LETTER lighter, heavier or flatter than the face it belongs to — all 124 glyphs, both styles | [docs/albo-weight-survey-2026-09-17.md](docs/albo-weight-survey-2026-09-17.md) — the ledger is the SCRIPT `tools/wedge_serif/cmp_weight_survey.py` (chamfer ridge, no percentile threshold). Grouped by CONSTRUCTION FAMILY, not by case: grouping by case flags the alphabet rather than the drawing. Headline findings (as of 2026-09-17; the roman `z` was re-cut in rounds 207/210 and now reads −9% of its family, so that one is history) — the italic `z` is the lightest glyph in the face; the italic runs 16% lighter than the roman with a −73% to +24% spread; the italic `X`'s 15-unit hairline and the roman `s`'s 16.6 are the two thinnest things that have to survive printing; the `7` is the lightest figure in both. Carries what was checked and found CLEAN, and the colour-measure bug that read an `f` at twice an `o` |
| The NUMERALS as OPTIONS — two to four per digit per style from the old-style references, env-selectable, the recommended sets, and what each option measures | [docs/albo-figure-options-2026-09-18.md](docs/albo-figure-options-2026-09-18.md) — `ALBO_FIG_0`…`ALBO_FIG_9` = `a\|b\|c\|d` in `figures.py` (the italic 7 alone takes `a`–`h` since round 231), all defaulting to today's drawing; a winner ships by one letter in `FIG_SHIP_ROM` / `FIG_SHIP_IT`. Its findings: Albo's 8 is 97 units short of its own 6 (every reference tops the 8 on the 6's line), the roman figures are flat (cuts 1.4–1.5 against 2–3), and six of ten sit on the width solver's 0.70 clamp |
| **What actually ships — every arm and dial, read from the builders themselves** | [docs/albo-STATE.md](docs/albo-STATE.md) — GENERATED by `tools/wedge_serif/gen_state.py`; `--check` fails when it is stale. Prose docs carry the history and go stale about defaults (on 2026-09-21 two of them named the wrong roman g); **if a doc disagrees with that table, the table is right** |
| Did anything move that we did not intend — gates, approved letters, a dial that is actually live | `tools/wedge_serif/gates.sh` (all gates against a tracked `gates-baseline.txt`, exit 1 only on a DELTA; `--accept` makes accepting a finding a reviewable commit) · `approved.py` (the owner's ruled letters, by outline — round 336 shipped a letter no ruling was made on and every gate was clean) · `ladder.py` (**refuses to render a ladder whose rungs are identical** — six dials were laddered dead on 2026-09-21) · `cmp_outlines.py` (a TTF's md5 never compares across builds; fontTools stamps `head.modified`) |
| Where did a number in a doc come from | [tools/wedge_serif/instruments/](tools/wedge_serif/instruments/) — the scripts that decided shipped geometry, moved out of the session scratchpad that deletes them. A surprising number is the instrument until proven otherwise, and that cannot be done against a file that is gone |
| Are there HAIRS, FRACTURES or STAIRCASES in a contour's own point stream — the layer above the raster the glitch sweep reads | `tools/wedge_serif/cmp_contour_hairs.py FONT.ttf [--letters]` — round 291, owner *"bold italic 700 has errors and glitches and fractures and hairs"*: exit 1 on a REVERSAL past 165 degrees, on a HAIR (a reversal past 150 whose shorter arm is under 8 units), or on DEGENERATE DENSITY (over a quarter of a glyph's segments under two units — the `s` read 59% and the `g` 48% where the face's `c e n o` read 0–6%, because `geom.close_corners` fanned a round buffer over an already-dense polyline). Coincident points are collapsed BEFORE any angle is taken, which is what makes it honest: a spike beside a duplicate point otherwise reads as 90 degrees, as nothing, or as a division by zero. `--letters` **is green on both styles as of round 367** -- 52 glyphs swept, 0 findings, exit 0, measured 2026-09-23. This line has now been wrong in BOTH directions: it first said "the arm held green" when the arm was red, was corrected on 2026-09-21 to name the roman's `h` and the italic's `b p y`, and that correction is what went stale when those four were fixed. **Run it rather than reading this sentence** -- which is the general case for every count in this file, and the reason `gates.sh` diffs a tracked baseline instead of trusting prose. The italic `y`'s hair arrived at round 297, whose commit says "every other font unchanged", and shipped as TestFlight build 205; the full-sweep arm (no `--letters`) still exits 1 on 14 pre-existing findings (`docs/albo-process-retrospective-2026-09-21.md`; the italic ampersand's ruled curl finding left that list at round 377, `docs/albo-ampersand-2026-09-24.md`) |
| Are any COUNTERS DENTED at a heavy weight — ink bulging into a counter's own convex hull, every glyph, before and after | `tools/wedge_serif/cmp_counter_dents.py A.ttf B.ttf --out DIR` — owner ruling 2026-09-19, *"for 700 and 900, counters should not have indentations"*; the 400s read none, the 700 nine and the 900 seventeen before round 270 made every ring's counter its own convex hull above stem 84. Exit 1 on any dent; the ampersand's lower loop is the one drawn exception |
| Where are the BUMPS — every deviation of an outline from its own local curve, circled and numbered on a hi-res sheet per style for the owner to confirm | `tools/wedge_serif/albo_bumps.py --roman R.ttf --italic I.ttf --out DIR` — a vertex is a bump when it leaves a circle fitted to its ±6 neighbours by over 1.2 units (a facet on a true arc leaves it by nothing; a wave by units), or kinks over 20° without being a corner; corners and their neighbours are excluded, one circle per 48 units of a wavy edge; `bumps-<style>.json` maps each number to a glyph and a position. Its two earlier detectors are recorded in the file: a turn-jolt threshold circled every facet (1,623), a moving-average residual lit every bowl (the average of an arc's neighbours lies inside it by the sagitta) |
| A COMPLETE SPECIMEN of both styles for the owner's review — reading sizes at integer magnification, a 40 px paragraph, every encoded glyph, and the word tests | `tools/wedge_serif/albo_specimen.py --roman R.ttf --italic I.ttf --out DIR` — PNGs at native pixels, nearest-neighbour for the magnified rows, per the proof-figure rules; the page that carries them is published from DIR each round |
| Which LETTERS do not fit the rest of the face — weight, spacing, proportion, defects — ranked, both styles, with what was checked and found CLEAN | [docs/albo-misfit-audit-2026-09-18.md](docs/albo-misfit-audit-2026-09-18.md) — a dated ledger from built fonts (italic round 222, roman round 221), grouped by construction FAMILY so the percentages are not the survey's case-grouped ones. Its headline: the italic `Y` +92% of the diagonals and the `g`'s descender a third shallower than every other; the roman `Q` (14 of the 19 touching pairs), `Z` +65%, and **the roman figures never received round 216's body fit** (spread 2.10× against the italic's 1.31×). Two premises it corrected: the roman `fi`/`ffi` are NOT glitch findings (declared islands), and the roman `z` is no longer the heaviest lowercase |
| What is each Aldine italic letter measured against, and which contrast arm is it on | [docs/albo-aldine-metrics.md](docs/albo-aldine-metrics.md) — the ledger is the SCRIPT `tools/wedge_serif/cmp_aldine_metrics.py`, which fails when a letter drifts >10% from its measured target; contrast is PER LETTER |
| How are Albo's CAPITALS SPACED, are any two letters TOUCHING, and against what | [docs/albo-capital-spacing.md](docs/albo-capital-spacing.md) — minimum white between two capitals in em, measured on the SHAPED pair so the kern table is in it; `tools/wedge_serif/cmp_cap_space.py` re-runs it. The baseline is not zero: Albo's capitals run a uniform +0.045 em looser than Flanker and that is the face's own rhythm, so a fault is distance from THAT. Carries the instrument's own bug — an unkerned placement hid two capitals that were touching. Its round-178 half is the COLLISION SWEEP: `tools/wedge_serif/cmp_touch.py` walks all 5,193 pairs and found 46 touching; it is a gate (non-zero exit) and records why a descender clash must be a kern pair rather than a wider fitting band |
| What width should Albo's capitals be, against Trajan, humanist and Helvetica | [docs/albo-capital-widths.md](docs/albo-capital-widths.md) — the separator is the ROUND-to-SQUARE ratio, not overall width; `tools/wedge_serif/cmp_capwidths.py` re-runs the measurement |
...
| RAKING LIGHT — tilt the phone and the letterpress deboss AND the sheet are lit from a moving lamp; the model, the edge-only relight, the half-resolution lamp field, the four sliders, measured before/after visibility and cost, and why the fixed light was always screen top-RIGHT on the phone | [docs/raking-light-spike-2026-09-25.md](docs/raking-light-spike-2026-09-25.md) — spike 2026-09-25, reworked 2026-09-26 (read that section first: the spike's edge-only relight moved 2.5% of pixels and the owner could not see it), device feel UNCONFIRMED |
| E-INK MODE — the light page as the X3's own e-paper panel: ghosting keyed on the firmware's FAST refreshes, the X3 waveforms decoded from its UC8253 LUTs (the HALF is a scrub, only FULL flashes), a Full Refresh gesture action, and the 7:1 cap on ghosts | [docs/eink-mode-spike-2026-09-25.md](docs/eink-mode-spike-2026-09-25.md) — spike, device feel UNCONFIRMED |
...
| The ZEN READING GOAL (was the daily reading allowance) — 5 minutes per zen session whatever the book, restarted when zen starts, the last minute decaying the page to unreadable; what counts, why the desktop ships it off | [docs/reading-allowance.md](docs/reading-allowance.md) — model `src/ReadingAllowance.h` (host-tested), pictures `src/SurfaceAllowance.h` |
...
| Letterpress — LIGHT pages only (doctrine 2026-08-22: light is paper and ink), **a 0..200 Settings.app slider again since 2026-09-11 (Ink > Impression), with the three press parts as their own sliders beside it** (owner: "ios app settings for rounding, spread and all other ink effects 0-200"; the 2026-08-23 freeze below is history), previously Off/Subtle/Standard/Heavy, ink-squeeze rim + deboss shadow + pressure + tooth, panel-space, darken-only. The pressure part's mapped range is WIDENED above 100% (`pressAmpScale`, 200% = 8x standard) — the 2026-08-22 audit found the dial near-dead, eaten by both the pixel math and a cache key that omitted the part percents | `src/Letterpress.h`, field built in `src/SurfaceSheet.cpp` and composited over the panel by `HalDisplay::presentIfNeeded`; `CROSSPOINT_SIM_LETTERPRESS` | `docs/letterpress-and-scanlines.md` |
| Ink rounding and ink spread — the TYPE's own corners and weight, not a surface over it: a blur-and-rethreshold on the four-level page image INSIDE the level-to-pixel conversion (the one pass allowed to remove ink), gain derived so straight edges hold, output still exactly four levels (2026-08-24 ruling kept). Light only. Two 0..200 sliders in Settings.app (Ink: Corner Rounding, Ink: Spread), shipping 27 and 106 since 2026-09-12 -- on the DESKTOP too, because the first-run `settings.json` carries them and is applied (owner ruling 2026-09-12, "desktop matches the phone everywhere"); `CROSSPOINT_SIM_INK_ROUNDING=0` is the bit-exact arm. Measured: rounding alone starves hairlines (Standard −12% page ink), spread pays it back; 42–70 ms per compose on the desktop after the near-ink mask, against 24 without | `src/InkRounding.h`, applied by `writePixelsFromLevels` in `src/HalDisplay.cpp`; `CROSSPOINT_SIM_INK_ROUNDING`, `CROSSPOINT_SIM_INK_SPREAD` | `docs/ink-rounding.md` |
...
**Settings.app is now twenty-seven groups and 64 rows** (the twenty-fourth to twenty-seventh are the **Raking** sliders, 2026-09-26 -- Strength, Lamp Height, Tilt Range, Page, one `PSSliderSpecifier` group each like the Ink six, `src/RakingLight.h` Dials; the twenty-third is **Speed Read**, 2026-09-25 -- a toggle that ships off plus a words-per-minute row, `src/SpeedRead.h`; the twenty-second is **E-Ink Mode**, 2026-09-25 -- one toggle, ships off, `src/EinkPanel.h`; the twenty-first is **Raking Light**, 2026-09-25 -- one toggle, ships off, `src/RakingLight.h`; the twentieth is **Reading Speedrun**, 2026-09-25 -- one toggle, `src/Speedrun.h`; the nineteenth is **Zen Reading Goal**, 2026-09-24 -- one multi-value row, `src/ReadingAllowance.h`; the eighteenth group is **The Left Margin**, 2026-09-21 -- six gesture rows, generated like the rest of the gesture half; it took the count from seventeen/48. Before it: the six Ink sliders arrived 2026-09-11 as one group and became six titled groups on 2026-09-12, because iOS draws a `PSSliderSpecifier` with NO title and the owner's screenshot showed six anonymous sliders -- a group header is the only label a slider can have; the count before it was already 42, not the 37 this sentence claimed -- the 2026-09-05/06 rocker and tilt rows had not been added to it) — count them out of
...
| Ink: Corner Rounding · Ink: Spread · Ink: Impression · Ink: Squeeze · Ink: Deboss Shadow · Ink: Plate Pressure | one `PSSliderSpecifier` each, 0..200; six groups because a slider has no title of its own. Ship 27 / 106 / 68 / 93 / 99 / 125 (owner 2026-09-12, his own slider positions). The Raking four below are the bundle's other sliders |
| Raking Light | Raking Light (Experimental) — toggle, ships OFF; tilt moves the lamp lighting the letterpress deboss AND the sheet itself (spike 2026-09-25, reworked 2026-09-26, `docs/raking-light-spike-2026-09-25.md`) |
| Raking: Strength · Raking: Lamp Height · Raking: Tilt Range · Raking: Page | one 0..200 `PSSliderSpecifier` each, all shipping 100 (2026-09-26, owner: "based on variable settings"), inert while the toggle above is off. Strength: the deboss shadow's depth over the fixed light's (100 = 3x). Lamp Height: the lamp's elevation at the neutral pose (0 = 12°, 100 = 35°, 200 = 58°). Tilt Range: degrees of lamp per degree of tilt (0 pins it). Page: how much of the paper budget the sheet's falloff and shaded relief spend. `src/RakingLight.h` Dials, dial-table rows `Raking*Percent` |
| E-Ink Mode | E-Ink Mode (Experimental) — toggle, ships OFF; the light page as the X3's own panel: ghosts across FAST refreshes, the decoded waveform on HALF/FULL, and a bindable **Full Refresh** gesture action (spike 2026-09-25, `docs/eink-mode-spike-2026-09-25.md`) |
