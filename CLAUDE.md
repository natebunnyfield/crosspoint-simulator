...
| Is any LETTER lighter, heavier or flatter than the face it belongs to — all 124 glyphs, both styles | [docs/albo-weight-survey-2026-09-17.md](docs/albo-weight-survey-2026-09-17.md) — the ledger is the SCRIPT `tools/wedge_serif/cmp_weight_survey.py` (chamfer ridge, no percentile threshold). Grouped by CONSTRUCTION FAMILY, not by case: grouping by case flags the alphabet rather than the drawing. Headline findings — the roman `z` is the heaviest lowercase and the italic `z` the lightest glyph in the face (73% apart); the italic runs 16% lighter than the roman with a −73% to +24% spread; the italic `X`'s 15-unit hairline and the roman `s`'s 16.6 are the two thinnest things that have to survive printing; the `7` is the lightest figure in both. Carries what was checked and found CLEAN, and the colour-measure bug that read an `f` at twice an `o` |
