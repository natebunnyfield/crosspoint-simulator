...
| 260 | (the same message) | SIX more fractions -- the thirds and the eighths, U+2153 U+2154 U+215B U+215C U+215D U+215E. Albo carried only the quarters and the half. They are built by `symbols2._fraction` from the shipping FIGURES, so every ruling on a digit reaches them and nothing can drift; no new drawing. |
| 258-259 | "continue autonomously on all commonly needed roman, italic, bold and bold italic characters" | TEN characters added, all in `marks.py` (the `@glyph` decorator encodes automatically, so no table edit). Nine are the face's own drawings at codepoints it did not carry: U+2010 HYPHEN and U+2011 NON-BREAKING HYPHEN and U+00AD SOFT HYPHEN (the hyphen; the soft one is what justified text with hyphenation needs), U+2012 FIGURE DASH (0.45 cap = advance 442, the mean of Albo\'s PROPORTIONAL old-style figures, 288 to 494 -- there is no tabular width to match), U+2015 HORIZONTAL BAR (the em dash, the quotation dash), U+201A and U+201E (the comma as the German and Czech opening quotes), U+02BC MODIFIER LETTER APOSTROPHE and U+02BB (the Hawaiian okina). The tenth, U+00B5 MICRO SIGN, is a new drawing: Albo\'s own u with its left stem run on below the baseline, thinning to 0.62 and cut at the family angle like the p\'s and q\'s. Coverage was measured first and was already strong -- every codepoint the reader\'s own corpus doc names was present. |
