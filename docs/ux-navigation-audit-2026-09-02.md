...
| 6 | **FIXED** 2026-09-02 (owner: "fix both") | `shipsInert(Gesture::Count)` now returns false, one guard line in `ios/GestureBindings.h`; `tests/gesture_bindings_test.cpp` asserts it. No foreign recognizer exists today, so nothing observable changes. |
| 7 | **FIXED** 2026-09-02 (owner: "fix both") | the three remaining README rows corrected: the shake row reads zen AND not-zen; "One row fires outside zen" is two (hold above, shake); the boundary sentence says the phone's `layoutPad` publishes both and the tablet falls back to `zenPaperBottomPx()`. The `:537` row was already corrected with finding 4. The "~194 pt" vs `unit=px` log note is left: both are true. |
