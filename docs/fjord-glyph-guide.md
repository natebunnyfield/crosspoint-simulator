...
| 2 | ONE stroke, chiselled: the round-49 arc continued by a STRAIGHT slash on the arc's own tangent (the arc's end angle solved so its tangent points at the base's left end), the arc at 0.82 of the profile and the slash growing to full by the base, the base bar 1.22 x the bar weight (`TWO_TOP_W`, `TWO_BASE_W`; owner: "rebalance 2 to be heavier on the bottom and lighter on the top"); the base the family's bar with its right wedge | round 81 |
...
| K | round 51's K (its traps) with the kick at 0.92 of its width at both ends (`K_KICK_TAPER`; owner: "K before is best for inktraps, but adopt some of C kick thickness") | round 81 |
| R | round 51's leg, no taper; the leg's start buried 0.28 of the cap stem into the bowl and thinned to 0.28 so its face lies inside the bowl's stroke (`R_LEG_BURY`; owner: "R counter is missing cleanup and the kick looks worse") | round 81 |
| Q | round 42's tail, its declared belly (1.05 stems) kept at 15% of its excess over the pen (`Q_BELLY`; owner: "leave Q as is, just reduce the bulge by 85%") | round 81 |
| M | round 51's M, its overshooting square faces clipped to the cap band (owner: "M after is worse ... try again") | round 81 |
| W | round 51's W, clipped at the cap line (owner: "just remove the tiny above triangle on top middle") | round 81 |
| S | the agent's bottom-left terminal at the top beak's 1.30 swell (owner: "keep S after") | round 77 |
| Z | the corners where the diagonal meets the bars: `Z_CORNER` 'blunt' (a bevel parallel to the diagonal, 0.45 S), 'mitre' (the bar run out to the diagonal's outer edge and cut on its line), 'wedge' (diagonal end wedges past the bars); blunt default, his pick pending (owner: "give Z a blunt edge and other similar more fitting connectors than a right angle") | round 81 |
