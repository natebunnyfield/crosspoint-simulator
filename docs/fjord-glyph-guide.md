...
| y | the left (thick) diagonal at `Y_LEFT_W` 0.97 of the pen's width for its angle, its outer edge and serif where they were, the tail untouched (owner 2026-09-14: "the 'y' is too dark in a word currently. thin out the left stroke of 'y' by decreasing its width, but leave the left side of the character as is"); the math's 0.80 put the y's knot -- the darkest 6 px window it makes in a word -- on the lowercase mean (Albertus -1.8%, Garamond +1.3%), but his eye at 500: "0.9 is slightly too thin for 500", and he picked 0.97 from a 0.92-0.98 ladder (knot about +6% over Albertus; the ruling is the eye's, the instrument records the gap); `outlines/cmp/balance.py` | rounds 88-90 |
...
| the capitals' bars | one unit, `pen.CAP_BAR` = 1.18 x the pen's horizontal = 0.68 of the cap stem (E F H L Z at 1.0, H 0.95, A 0.9, T 0.85, G 0.8; E F middle bars 0.9 of it as before). Measured on 1000 px rasters: Albo's were 0.47 of the cap stem (the pen's horizontal, 55), Albertus's 0.75, Berkeley's 0.39; at 13 pt they dropped to gray while the stems held black. Owner: "the crossbar for 'F' is too thin, take a pass at all capitals" | round 94 |
| a (hood) | `A_HOOD_W` 0.92 on the hood's stroke, outer run-then-arc and underside alike (owner: "slightly reduce the top stroke of 'a'") | round 94 |
| r (fit) | arm reach `R_REACH` 190 wf (205), floor `R_FLOOR` 0.72 S (0.78), flare `R_FLARE` 1.35 (1.5) -- owner: "reduce 'r' slightly so it fits with the rest of the chars"; read as size and weight together, the shape unchanged | round 94 |
| " and the curly doubles | the two marks `DQ_GAP` 1.8 stems apart, center to center (1.3: a 48-unit gap, 2.6 px at 13 pt, gray between); owner: "give more space for double quotes so they don't touch" | round 94 |
