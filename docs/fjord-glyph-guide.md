...
| 1 | the flag's centerline ends buried 0.35 TH_V inside the stem (it ran 2 above the top, a nub); its tip takes a SMALL diagonal end wedge, 0.32 of the family's (`ONE_FLAG_WEDGE_SCALE`; owner: "1 needs a much smaller tip serif at top") (it was the only free terminal among the figures with no serif) | round 77 |
| 2 | ONE stroke: the round-49 arc (to -20 deg) continued by a cubic that leaves on the arc's tangent, bows down-left and arrives horizontal into the base's left end; widths on the bowl profile, 1.15 at the start, the slash 0.72 (`TWO_ARC_END_DEG`, `TWO_SLASH_LEAVE/LAND/W`); the base the family's bar with its right wedge, no cut. Owner: "flow the top right into the slash into the bottom" | round 79 |
...
| 4 | OPEN with a CURVED top-left stroke (`FOUR_CURVED`; owner: "research how open 4 numerals can be done with curved top left stroke"): a cubic from a start clear of the stem's edge by 0.62 S + its half width, bowing left 0.22 S (`FOUR_BOW`), arriving at the bar's left end running along it; bowl-profile widths, thin at the pen-cut start. Precedent measured on disk: Berkeley Oldstyle, Miju Goudy and Cheltenham bow it and round it into the bar; Dante, EB Garamond, Doves and Van den Keere keep a straight thin diagonal; all stand clear of the stem. The straight open and the closed constructions are kept behind the constants | round 79 |
