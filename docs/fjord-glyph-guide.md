...
| 1 | the flag's centerline ends buried 0.35 TH_V inside the stem (it ran 2 above the top, a nub); its tip takes the family's 0.9 x 0.9 diagonal end wedge (it was the only free terminal among the figures with no serif) | round 77 |
| 2 | the arc eases to the diagonal's width over its last 18% and the diagonal is buried 0.44 S (the arc arrived at twice the diagonal's width, square-faced: a spur and a nick); the base's right end is the wedge alone, no pen cut (`TWO_BASE_CUT = False`; a left wedge was tried and rejected, it notched the diagonal's foot) | round 77 |
...
| 4 | OPEN (`FOUR_OPEN`): the diagonal's top slides down its axis until its face clears the stem by 0.62 S = 52 units (the 1.2 ink spread and the facets eat 4 of a nominal 0.6 S); the counter is no longer a hole; the closed code kept | round 77 |
