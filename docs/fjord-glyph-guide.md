...
| 1 | the flag's centerline ends buried 0.35 TH_V inside the stem (it ran 2 above the top, a nub); its tip takes a SMALL diagonal end wedge, 0.32 of the family's (`ONE_FLAG_WEDGE_SCALE`; owner: "1 needs a much smaller tip serif at top"); the flag itself on the bowl profile at 0.62 S so it is solid, not a pen hairline (it was the only free terminal among the figures with no serif) | round 77 |
| 2 | ONE stroke, chiselled: the round-49 arc continued by a STRAIGHT slash on the arc's own tangent (the arc's end angle is solved so its tangent points at the base's left end), the slash at the arc's weight (bowl profile, floor 0.80 S), its end buried in the base; the base the family's bar with its right wedge, the corner crisp. Owner: "flow the top right into the slash into the bottom"; a swash version was rejected: "lacking the understanding of what we're doing with this font" | round 80 |
...
| @ | the conventional @ (owner: "use an italic 'a' and connect the bottom right to the loop to its right like a conventional"): a single-storey a slanted 6 deg, its bowl 0.5 of the interior's half width, its stem at 0.63 rx right of the ring's centre (in the spiral's open side, as Berkeley and Albertus); the tail leaves the stem's foot straight down and hooks LEFT into the spiral, which begins a little left of the foot (`AT_TAIL_OUT` -0.08) and sweeps 262 deg clockwise to end at one o'clock, clear of the stem top; everything on the bowl profile. Found the hard way: a start right of the foot forces the tail to cusp, because the spiral's travel there is down-left | round 80 |
| ? | seven variants built for the pick (`QUESTION_VARIANTS`, `FJORD_Q_VARIANT`): 0 the round-77 hook (landed), 1 the same on the bowl profile, 2 garalde wide, 3 tall narrow, 4 beak terminal, 5 Albertus heavy, 7 curled terminal (a stem-foot 6 was built and dropped: a claw) | round 80 |
