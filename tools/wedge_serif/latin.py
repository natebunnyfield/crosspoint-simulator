...
# ---------------------------------------------------------------- helpers
def capH(c): return c["asc"] * 0.94
def figH(c): return capH(c) * 0.92

def vstem(c, P, x, y0, y1, top_sides=(1, -1), foot_sides=(-1, 1), thin=1.0, flare=True):
    """A capital's stem: bracketed wedges BOTH sides top and foot."""
...
def g_B(c):
    P = []; C = capH(c); s = c["s"]; x = s / 2; w = 470 * c["wf"]
...
def g_C(c):
    P = []; C = capH(c); rx = 300 * c["wf"]; ry = C / 2 + c["over"]
...
def g_N(c):
    P = []; C = capH(c); s = c["s"]; x0 = s / 2; w = 560 * c["wf"]; x1 = x0 + w
    vstem(c, P, x0, 0, C, top_sides=(1,), foot_sides=(-1, 1), thin=0.72)
    diag(c, P, (x0, C), (x1, 0))
    vstem(c, P, x1, 0, C, top_sides=(-1, 1), foot_sides=(), thin=0.72); return P

def g_O(c):
    P = []; C = capH(c); rx = 330 * c["wf"]; cp_ring(c, P, rx, C / 2, rx, C / 2 + c["over"]); return P

def g_P(c):
    P = []; C = capH(c); s = c["s"]; x = s / 2; w = 450 * c["wf"]
    vstem(c, P, x, 0, C, top_sides=(1,), foot_sides=(-1, 1))
...
def g_S(c):
    P = []; C = capH(c); w = 480 * c["wf"]; o = c["over"]
...
# ---------------------------------------------------------------- figures (lining)
def g_zero(c):
    P = []; D = figH(c); rx = 230 * c["wf"]; cp_ring(c, P, rx, D / 2, rx, D / 2 + c["over"]); return P
...
def g_two(c):
    P = []; D = figH(c); s = c["s"]; w = 440 * c["wf"]; rx = w * 0.46
...
def g_three(c):
    P = []; D = figH(c); w = 430 * c["wf"]; r1 = D * 0.24; r2 = D * 0.28
    top = ellipse(w * 0.45, D - r1, r1 * 1.05, r1, math.radians(175), math.radians(-60), 50, c["k"])
    bot = ellipse(w * 0.45, r2, r2 * 1.15, r2 + c["over"], math.radians(60), math.radians(-175), 60, c["k"])
    A.curve(c, P, top, flare_end(0.2, 0.15), cut0=c["cut"])
    A.curve(c, P, bot, compose(taper_in(0.6, 0.1), flare_end(0.25, 0.15)), cut1=c["cut"]); return P

def g_four(c):
    P = []; D = figH(c); s = c["s"]; w = 480 * c["wf"]; xs = w * 0.7
    diag(c, P, (xs - s * 0.2, D), (s * 0.1, D * 0.3), thin=0.75)
    bar(c, P, 0, w, D * 0.3)
    vstem(c, P, xs, 0, D, top_sides=(), foot_sides=(-1, 1)); return P

def g_five(c):
    P = []; D = figH(c); s = c["s"]; w = 430 * c["wf"]; r = D * 0.31
    bar(c, P, s * 0.3, w * 0.95, D, serif_ends=[('right', -1)])
    vstem(c, P, s * 0.3 + s / 2, D * 0.5, D, top_sides=(), foot_sides=(), thin=0.85, flare=False)
    bowl = ellipse(w * 0.45, r, r * 1.2, r + c["over"], math.radians(125), math.radians(-160), 70, c["k"])
    A.curve(c, P, bowl, compose(taper_in(0.55, 0.12), flare_end(0.25, 0.15)), cut1=c["cut"]); return P

def g_six(c):
    P = []; D = figH(c); rx = 230 * c["wf"]; r = D * 0.29
...
def g_seven(c):
    P = []; D = figH(c); s = c["s"]; w = 440 * c["wf"]
    bar(c, P, 0, w, D, serif_ends=[('left', -1)])
    diag(c, P, (w - s * 0.2, D), (w * 0.3, 0)); return P

def g_eight(c):
    P = []; D = figH(c); rx = 215 * c["wf"]; r1 = D * 0.235; r2 = D * 0.265
...
def g_nine(c):
    ...
    P = []; D = figH(c); rx = 230 * c["wf"]; r = D * 0.29; s = c["s"]
...
def g_period(c): s = c["s"]; return [blob((s * 0.55, s * 0.55), s * 0.55)]
def g_comma(c):
...
def g_hyphen(c): return [A.outline(line((0, c["xh"] * 0.48), (240 * c["wf"], c["xh"] * 0.48), 8), c["pen"])]
def g_endash(c): return [A.outline(line((0, c["xh"] * 0.48), (500 * c["wf"], c["xh"] * 0.48), 8), c["pen"])]
def g_emdash(c): return [A.outline(line((0, c["xh"] * 0.48), (1000 * c["wf"], c["xh"] * 0.48), 8), c["pen"])]
def g_parenleft(c):
...
def g_numbersign(c):
    ...
    return P
def g_at(c):
    xh = c["xh"]; C = capH(c); w = 760 * c["wf"]; cx, cy = w * 0.5, xh * 0.5; P = []
    R = _round17()
