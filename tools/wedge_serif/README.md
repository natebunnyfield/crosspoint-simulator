...
| `outlines/build.py` — the bench tables | `ROM_LC_ADJ`, `ROM_PUNCT_ADJ`, `ALD_LC_ADJ`, `ALD_PUNCT_FIT`: (lsb, rsb) deltas per glyph, fitted from the owner's 329 bench judgments as ONE ridge-regularised linear system (round 308). Re-fit from the bench; never hand-tuned. `bench_values.json` holds the raw judgments in both readings. Method and the four instrument bugs: `docs/albo-spacing-method.md`. |
