...
| File | What |
|---|---|
| `outlines/kern.py` | **Albo's kerning** (round 95, ruled "hand class kerning"): LEFT/RIGHT classes, `CLASS_PAIRS` and `PAIRS` in 18-unit steps, written as a GPOS `kern` feature by feaLib; `outlines.build` applies it to every build, `python3 -m outlines.kern <ttf> [<out>]` re-applies to an existing file in seconds. `reader_view()` runs the firmware's own extractor at 54 px. Also carries the `liga` feature, which since round 310 is **roman-only and is exactly ff fi fl ffi ffl** — the italic's is empty by ruling. **Two claims in its docstring were corrected on 2026-09-20:** the quantum is 1.16 design units on the phone and 2.31 on the X3, not 18.5 and 37 (those are one whole PIXEL; the format is 4.4 fixed point), and "the lowercase is deliberately unkerned" is now a statement about the KERN TABLE only — the lowercase is fitted from the owner's bench in `build.py`, not here. |
| `bench_fit.py` | **The fit itself, as a script** (round 344). Reads `bench_values.json`, solves both bearings of every glyph as ONE ridge-regularised system, solves the marks directly, turns the leftover capital pairs into kerns, and prints the four tables `build.py` ships. `--check` fails when `build.py` no longer matches the bench. **`--drop g` is the default**, owner 2026-09-21: *"ignore new 'g' values because it was with old g"*. Its docstring is the method; it was validated by reproducing round 308's shipped tables. |
| `outlines/build.py` — the bench tables | `ROM_LC_ADJ`, `ROM_PUNCT_ADJ`, `ALD_LC_ADJ`, `ALD_PUNCT_FIT`: (lsb, rsb) deltas per glyph, fitted from the owner's **384** bench judgments as ONE ridge-regularised linear system (round 308, refitted round 344). **Never hand-edit these — run `bench_fit.py`.** `bench_values.json` is a checkout of the bench's own database, in both readings; refresh it from the Artifact page before fitting. Method and the five instrument bugs: `docs/albo-spacing-method.md`. |
| `build_env.sh` | The shipping environment for both styles, in one place: `source build_env.sh && albo_build OUTDIR`. A bare `python3 -m outlines.build DIR --style Italic` builds a DIFFERENT font that looks like Albo, with `_ALD.ON` false — it cost a before/after proof on 2026-09-21 that read `Foot` at +9 units where the real change is −41. `gates.sh` keeps its own copy on purpose. |
| `pair_census.py` | **What his books actually contain.** Counts every letter pair across the epubs under `~/src/claude-tools` (36 books, 2,007,794 pairs) and names the commonest real word carrying each. It chose the bench's 396 rows and it is what rejected four drawn ligatures. `ALBO_PAIR_OUT` picks the output file. |
| `gap_measure.py` | The white between two glyphs: `rsb(a) + kern + lsb(b)`. Use it, and not the sum of the two advances — that counts the right glyph's far side and reads a mark's two bearings as one (it reported −77 for a −44 change in round 303). |
| `outlines/glyphs/ligatures.py` | fi fl ff ffi ffl (round 96) on the f's parts (`stems.f_ink` with a re-aimed hook); `FI_PUSH` / `FL_PUSH` / `FF_STEP` place the following stem. Reached by cmap U+FB00-FB04 and the `liga` feature in `outlines/kern.py`. **Round 309 added six more and ships none of them:** `fb fh fj fk` at U+E000-E003 (drawn, then rejected on the corpus — 13 occurrences in two million pairs), `st` at U+FB06 and `ct` at U+E004 (wanted badly, but the joining arc reads as a spur off the s rather than a span to the t), and `Th` at U+E005 (shipped one round, withdrawn on the owner's eye). Each is drawn and reachable by codepoint; switching one on is one line in `kern.py`. `_place()` composes a ligature's second half from the glyph registry, so it cannot drift from the letter it replaces. |
...
| `outlines/glyphs/greek_italic.py` | **The italic's cursive Greek** (round 379), live only under `ALBO_ITALIC=aldine`. The η μ ν ι α τ π κ υ ψ σ ρ are built from the Aldine letters' own parts (`a_n`, `a_u`, `a_v`, `hm_stem`/`hm_head`/`hm_exit`, the o's ring); the rest re-run `symbols2.py`'s traced roman skeletons inside `symbols2.hand(ItalicHand)`, which routes their five helpers to the italic's ring, nib, stem and finial. The roman Greek, the 15 composite copies (`build.GREEK_COPY`) and the spacing (`build.GREEK_SIDES`, `GREEK_SIDE_ADJ`, fitted by `instruments/greek376_space.py --fit`) are in `docs/albo-greek-2026-09-23.md`, round 379. |
