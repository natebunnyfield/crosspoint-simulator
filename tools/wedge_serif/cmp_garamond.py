...
FJ = TTFont('fonts/Fjord-Regular.ttf'); GA = TTFont('ref/EBGaramond-400.ttf')
def info(font):
    return dict(xh=font['OS/2'].sxHeight, cap=font['OS/2'].sCapHeight, gs=font.getGlyphSet(), cmap=font.getBestCmap(), hmtx=font['hmtx'])
F, G = info(FJ), info(GA)
...
page = f'''<title>Fjord against Garamond</title>
<style>
...
<h1>Fjord against Garamond, glyph by glyph</h1>
<p>Each cell: <b>EB Garamond 400</b> (old-style figures, its <code>onum</code> set) (gray) · <b>Fjord-Regular</b> (black) · both overlaid (Garamond red). Both fonts scaled so their x-heights match (Fjord 415, Garamond 400 units); baseline solid, x-height dashed; glyphs aligned on the left edge of their ink. Numbers are in x-heights; stroke = 2·area/perimeter, a mean stroke thickness.</p>
...
<ul style="max-width:80ch"><li>Lowercase mean stroke: Fjord is <b>{100*(GLOBAL['low_thick']-1):+.0f}%</b> against Garamond 400 (median over a–z); capitals <b>{100*(GLOBAL['cap_thick']-1):+.0f}%</b>.</li>
<li>Capital height over x-height: Fjord {GLOBAL['capxh_f']:.2f}, Garamond {GLOBAL['capxh_g']:.2f} — every capital's top sits about {GLOBAL['cap_top']:+.2f} xh above Garamond's, which is why most of A–Z carry a "top" flag.</li>
<li>Measured on the raster: <b>l</b> stem 0.195 xh (Garamond 0.175, +11%); <b>o</b> hairline 0.128 xh (Garamond 0.075, <b>+70%</b>); o side 0.182 (Garamond 0.205). Contrast 1.4 against 2.7 — the mean stroke matches Garamond but the thins are far too heavy and the thicks a little light, which is what makes the page read bolder.</li>
...
'''
open('fonts/fjord-vs-garamond.html', 'w').write(page)
print(len(rows), 'glyphs;', len(ranked), 'flagged')
for r in ranked: print(f'{r["ch"]!r:5} score {r["score"]:.2f}  ' + '; '.join(r['flags']))
print('--- global: Fjord cap/xh', F['cap']/F['xh'], ' Garamond', G['cap']/G['xh'])
